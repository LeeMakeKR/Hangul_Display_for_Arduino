...
초성 섹션: 0~159 (20자 × 8벌)
  인덱스 0: 비어있음
  인덱스 1~19: ㄱ ㄲ ㄴ ㄷ ㄸ ㄹ ㅁ ㅂ ㅃ ㅅ ㅆ ㅇ ㅈ ㅉ ㅊ ㅋ ㅌ ㅍ ㅎ
  
중성 섹션: 160~247 (22자 × 4벌)
  인덱스 0: 비어있음
  인덱스 1~21: ㅏ ㅐ ㅑ ㅒ ㅓ ㅔ ㅕ ㅖ ㅗ ㅘ ㅙ ㅚ ㅛ ㅜ ㅝ ㅞ ㅟ ㅠ ㅡ ㅢ ㅣ
  
종성 섹션: 248~359 (28자 × 4벌)
  인덱스 0: 비어있음 (종성 없음)
  인덱스 1~27: ㄱ ㄲ ㄳ ㄴ ㄵ ㄶ ㄷ ㄹ ㄺ ㄻ ㄼ ㄽ ㄾ ㄿ ㅀ ㅁ ㅂ ㅄ ㅅ ㅆ ㅇ ㅈ ㅊ ㅋ ㅌ ㅍ ㅎ
```
...
**⚠️ 중요**: 폰트 파일은 **벌 단위로 묶여있습니다** (벌 × 개수 + 인덱스).

```cpp
// 초성 글리프 인덱스 (벌 × 20자 + 초성인덱스)
// cho: 1~19 (조합형 코드), cho_bul: 0~7 (벌-1)
cho_glyph_index = cho_bul * 20 + cho;

// 중성 글리프 인덱스 (오프셋 160 + 벌 × 22자 + 중성인덱스)
// jung: 1~21 (조합형 코드), jung_bul: 0~3 (벌-1)
jung_glyph_index = 160 + jung_bul * 22 + jung;

// 종성 글리프 인덱스 (오프셋 248 + 벌 × 28자 + 종성인덱스)
// jong: 0~27 (조합형 코드, 0=종성없음), jong_bul: 0~3 (벌-1)
jong_glyph_index = 248 + jong_bul * 28 + jong;
```

**인덱스 변환 예시**:
```
'안' = 초성 ㅇ(12), 중성 ㅏ(1), 종성 ㄴ(4)
  → 벌: 초성 6벌, 중성 4벌, 종성 1벌
  → 초성 글리프: 5 × 20 + 12 = 112
  → 중성 글리프: 160 + 3 × 22 + 1 = 227
  → 종성 글리프: 248 + 0 × 28 + 4 = 252
```
...
`han_to_h.py`와 `render_hangul.py`는 EasyView 폰트를 테스트하고 시각화하기 위한 Python 도구입니다.

#### 도구 파일
- **`han_to_h.py`**: `.han` 폰트를 Arduino `.h` 헤더 파일로 변환
- **`render_hangul.py`**: 한글 텍스트를 이미지로 렌더링

### 7.2 han_to_h.py - 폰트 변환기

EasyView 폰트 파일(`.han`)을 Arduino/ESP32용 C 헤더 파일로 변환합니다.
...
**예시**:
```bash
python han_to_h.py ../EasyView-font/ko/H01_kr.han
# 출력: H01_kr.h
```
...
typedef struct {
  ...
  uint8_t width, height, bytesPerGlyph;
  uint16_t totalGlyphs;
  uint8_t choOffset, jungOffset;
  uint16_t jongOffset;
} HangulFontInfo;

// 헬퍼 함수들
const uint8_t* H01_kr_getGlyph(uint16_t index);
const uint8_t* H01_kr_getCho(uint8_t bul, uint8_t index);
const uint8_t* H01_kr_getJung(uint8_t bul, uint8_t index);
const uint8_t* H01_kr_getJong(uint8_t bul, uint8_t index);
```

### 7.3 render_hangul.py - 한글 렌더러

UTF-8 한글 텍스트를 조합형 폰트로 렌더링하여 PNG 이미지로 출력합니다.
...
**사용법**:
```bash
python render_hangul.py <font.han> <text> [output.png]
```

**예시**:
```bash
python render_hangul.py ../EasyView-font/ko/H01_kr.han "안녕하세요" output.png
```
...
def decompose_hangul(self, char):
    ...
    # 폰트 파일 인덱스로 변환 (인덱스 0은 비어있음)
    cho_index = cho_unicode + 1   # 1~19
    jung_index = jung_unicode + 1  # 1~21
    jong_index = jong_unicode      # 0~27 (0=종성없음)
...
def get_bul_indices(self, cho_index, jung_index, jong_index):
    """초성, 중성, 종성의 벌 인덱스 계산"""
    jung_char = JUNG_TABLE[jung_index - 1]
    ...
    # 중성 벌 (0~3): 초성 ㄱ/ㅋ 여부와 받침 유무
    is_giyeok_or_kieuk = (cho_index == 1 or cho_index == 16)
    jung_bul = 0 if (not has_jong and is_giyeok_or_kieuk) else \
               1 if (not has_jong) else \
               2 if (is_giyeok_or_kieuk) else 3
...
```python
# 초성 글리프 인덱스
cho_glyph_index = CHO_OFFSET + cho_bul * CHO_COUNT + cho_index

# 중성 글리프 인덱스  
jung_glyph_index = JUNG_OFFSET + jung_bul * JUNG_COUNT + jung_index

# 종성 글리프 인덱스
jong_glyph_index = JONG_OFFSET + jong_bul * JONG_COUNT + jong_index
```

### 7.5 트러블슈팅

#### 문제: 중성이 한 글자씩 밀림
**원인**: 폰트 파일의 중성 섹션은 인덱스 0만 비어있음 (0, 1 두 개가 비어있지 않음)  
**해결**: `jung_index = jung_unicode + 1` (기존: `+ 2`)

#### 문제: 받침 있는 글자의 중성/종성 위치가 안 맞음
**원인**: 벌 선택 규칙이 레퍼런스와 다름  
**해결**: 레퍼런스의 정확한 벌 선택 규칙 구현 (초성 1~8벌, 중성 1~4벌, 종성 1~4벌)
...
### 주요 구현 포인트
- ✅ 중성 인덱스: `유니코드 중성 + 1` (인덱스 0만 비어있음)
- ✅ 초성 벌: 중성 종류와 받침 유무로 1~8벌 결정
- ✅ 중성 벌: 초성 종류(ㄱ/ㅋ)와 받침 유무로 1~4벌 결정  
- ✅ 종성 벌: 중성 종류로 1~4벌 결정
- ✅ 글리프 인덱스: `오프셋 + 벌 × 개수 + 인덱스` (벌 단위 배치)
