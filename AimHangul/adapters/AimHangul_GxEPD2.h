...
void AimHangul_base (uint16_t XPOS,uint16_t YPOS,const String& hgStr,uint32_t color,uint16_t hgSize) { 
  ...
  #elif defined(_GxEPD2_H_)
  if ((display.getRotation() % 2) == 0) {
    screenWidth  = 400; //width, height는 해상도에 맞게 수정해야됨
    screenHeight = 300;
  }
  else {
    screenWidth  = 300; //width, height는 해상도에 맞게 수정해야됨
    screenHeight = 400;
  }
...
//      tft.drawXBitmap(XPOS, YPOS, pHgImg2, w1, h1, color);
      spr.drawXBitmap(XPOS, YPOS, pHgImg2, w1, h1, color); //sprite적용을 위해 윗줄 수정
      #elif defined(_GxEPD2_H_)
      display.drawXBitmap(XPOS, YPOS, pHgImg2, w1, h1, color); //GxEPD2 라이브러리 사용
      #elif defined(_U8G2LIB_HH)
...
//      tft.drawXBitmap(XPOS, YPOS,pHgImg2, w2, h2, color);
      spr.drawXBitmap(XPOS, YPOS,pHgImg2, w2, h2, color); //sprite적용을 위해 윗줄 수정
      #elif defined(_GxEPD2_H_)
      display.drawXBitmap(XPOS, YPOS, pHgImg2, w2, h2, color); //GxEPD2 라이브러리 사용
      #elif defined(_U8G2LIB_HH)
